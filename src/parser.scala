...
object SimpleParser extends Parsers {
  ...
  class TR(val all: Seq[Token]) extends Reader[Token] {
    ...
    override def rest = new TR(all.drop(all.indexOf(first) + 1))
    def remaining: Int = eff.size
  }

  def anyOf(ps: Parser[AstExpr]*): Parser[AstExpr] = Parser(in => {
    val rs = ps.map(_(in)).collect { case Success(r, n: TR) => (r, n) }
    if (rs.isEmpty) Failure("", in) else { val m = rs.maxBy(x => x._2.remaining * -1); if (rs.size == 1) Success(m._1, m._2) else Success(Ambiguous(rs.map(_._1).toList), m._2) }
  })

  private def id = accept("id", { case TV(n) => n })//これはIdentifierの略
  private def nm = accept("num", { case TN(v) => v })//これはNumberの略
  private def o(s: String) = accept(s"o $s", { case TO(`s`) => s })//これはOperatorの略
  private def k(s: String) = accept(s"k $s", { case TK(`s`) => s })//これはKeywordの略
  private def s(s: String) = accept(s"s $s", { case TS(`s`) => s })//これはSymbolの略
  ...
  def logicOr: Parser[AstExpr] = logicAnd ~ rep(o("||") ~ logicAnd) ^^ { case t ~ l => l.foldLeft(t) { case (x, op ~ y) => BinOp(op, x, y) } }
  def logicAnd: Parser[AstExpr] = compare ~ rep(o("&&") ~ compare) ^^ { case t ~ l => l.foldLeft(t) { case (x, op ~ y) => BinOp(op, x, y) } }
  def compare: Parser[AstExpr] = arith ~ rep((o("==") | o("<") | o(">")) ~ arith) ^^ { case t ~ l => l.foldLeft(t) { case (x, op ~ y) => BinOp(op, x, y) } }
  def arith: Parser[AstExpr] = term ~ rep((o("+") | o("-") | o("<<") | o(">>")) ~ term) ^^ { case t ~ l => l.foldLeft(t) { case (x, op ~ y) => BinOp(op, x, y) } }
