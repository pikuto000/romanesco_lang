...
with PackratParsers
{
  ...
  private def Stream(input:Input): ParseResult[Array[Token]] = {
    ...
    lazy val tokenizer: PackratParser[Token] ={
      input => {
    ...
    val parseRes = phrase(rep(tokenizer)) (input) match {
  ...
  class d(tag:HygenicTag) extends HygenicObj(tag) {
    ...
    def whoAreYou(w:Input):Option[Parser[Token]]={
      ...
      var bestTag: Option[HygenicTag] = None
      
      def updateBest(t: HygenicTag, p: Parser[Token], res: Token): Unit = {
        ...
        logger.log(s"rule ${t.name} succeeded. Length: $len")
        ...
        if (bestParser.isEmpty) {
          ...
          if (len > maxLen) {
            ...
            logger.log(s"rule ${t.name} length ($len) == maxchamp rule ${bestRuleName} length ($maxLen). Compare RuleOrder.")
            // Earlier rule (smaller index) wins.
            if (RuleOrder(t) < RuleOrder(bestTag.get)) {
              logger.log(s"rule ${t.name} order (${RuleOrder(t)}) < maxchamp rule ${bestRuleName} order (${RuleOrder(bestTag.get)}). Update.")
              bestRuleName = t.name
              bestTag = Some(t)
              bestParser = Some(p)
            } else {
              logger.log(s"rule ${t.name} order (${RuleOrder(t)}) >= maxchamp rule ${bestRuleName} order (${RuleOrder(bestTag.get)}). Keep maxchamp.")
            }
          } else {
            logger.log(s"rule ${t.name} length ($len) < maxchamp rule ${bestRuleName} length ($maxLen). Ignored.")
          }
        }
      }
      ...
      for ((s, p) <- definedTokenizeRules) {
        ...
        p(w) match {
          ...
          case NoSuccess(_, _) => 
          logger.log(s"rule ${s.name} failed.")
        }
      }
    ...
    def dumpDataBase:Array[(HygenicTag, Parser[Token])]=definedTokenizeRules.toArray
  }
