...
class Solver extends AutoCloseable {
  ...
  private val lbs = mutable.Map[BoolExpr, Stmt]()
  ...
  private def b(e: AstExpr, env: Map[String, Expr[RealSort]]): BoolExpr = e match {
    ...
    case BinOp("&&", l, r) => ctx.mkAnd(b(l, env), b(r, env)); case BinOp("||", l, r) => ctx.mkOr(b(l, env), b(r, env))
    case UnOp("!", ex) => ctx.mkNot(b(ex, env)); case _ => ctx.mkNot(ctx.mkEq(toZ(e, env), ctx.mkReal(0)))
  }
  ...
  def add(st: Stmt): Unit = st match {
    case Constraint(Var(n), Lambda(p, bd)) =>
      ...
      fs += (n -> f)
    case Constraint(l, r) => val lb = ctx.mkFreshConst("L", ctx.getBoolSort).asInstanceOf[BoolExpr]; lbs += (lb -> st); s.add(ctx.mkImplies(lb, ctx.mkEq(toZ(l), toZ(r))))
    case Verify(e) => val lb = ctx.mkFreshConst("L", ctx.getBoolSort).asInstanceOf[BoolExpr]; lbs += (lb -> st); s.add(ctx.mkImplies(lb, b(e, Map.empty)))
    case Block(ss) => ss.foreach(add); case Branch(o) => s.add(ctx.mkOr(o.map(convert)*))
    case _ =>
  }
  ...
  def check() = s.check(lbs.keys.toSeq*)
  def getCore() = s.getUnsatCore.toList.flatMap(lbs.get)
  def model = s.getModel; def close() = ctx.close()
}