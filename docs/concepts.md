...
- **`_` (Wildcard)**: 「今はどうでもいい」ことを表す受動的な不在。制約のスキップや、カリー化時の位置確保に使用。
...
- **構造的適用 (Structural Apply)**:
    括弧内での並置 `(f x)` は、名前を持たない構造的な `Apply` ノードとしてパースされ、動的な関数適用として機能する。
- **動的アリティ登録**:
    `:-` によって関数が定義された際、シンボルテーブル上でその名前は自動的に「1引数オペレータ」として登録され、以降のパース規則に即座に影響を与える。
