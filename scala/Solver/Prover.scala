...
final class Prover(val classical: Boolean = false) {
  ...
  private def freshMeta(depth: Int): Expr = {
    metaCounter += 1
    logger.log(s"meta counter: $metaCounter")
  ...
  def prove(
    ...
    logger.log(
      s"prove begin. goal: $goal (classical: $classical, maxDepth: $maxDepth)"
    )
  ...
  private def searchAxiom(
    ...
    context.view.to(LazyList).flatMap { case (name, hyp) =>
      unify(hyp, goal, subst)
        .map(s => (List(ProofStep.Apply(CatRule(name, hyp, hyp), s)), s))
    }
  ...
  private def searchDecomposeGoal(
    ...
    goal match
      case Expr.App(Expr.Sym(And | Product), List(a, b)) =>
        ...
        yield (
          proofA ++ proofB :+ ProofStep.Apply(
            StandardRules.productUniversal,
            s2
          ),
          s2
        )
      ...
      case Expr.App(Expr.Sym(op), List(a, b))
        ...
        search(
          ...
          .map { case (proof, s) =>
            (proof :+ ProofStep.Apply(StandardRules.expUniversal, s), s)
          }
      ...
      case Expr.App(Expr.Sym(Forall), List(Expr.Var(v), body)) =>
        ...
        search(
          ...
          .map { case (proof, s) =>
            (proof :+ ProofStep.Apply(StandardRules.forallCounit, s), s)
          }
        ...
        def solveUniversals(
          ...
          case head :: tail =>
            search(
              ...
              visited,
              raaCount
            ).flatMap { case (_, nextSubst) =>
              solveUniversals(tail, nextSubst)
            }
        }
  ...
  private def searchClassical(
    ...
    if classical && raaCount < 1 && goal != Expr.Sym(False) && goal != Expr.Sym(
        Initial
      )
    then
      ...
      search(
        ...
        .map { case (p, s) =>
          val raaStep = ProofStep.Apply(
            CatRule(
              "RAA",
              Expr.App(Expr.Sym(Implies), List(negation, Expr.Sym(False))),
              goal
            ),
            s
          )
