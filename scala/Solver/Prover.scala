...
object Prover {
  ...
  def prove(
    ...
    metaCounter = 0
    search(goal, rules, Nil, emptySubst, 0, maxDepth, Set.empty).map(_._1)
  ...
  private def searchDecompose(
    ...
    goal match
      ...
      case Expr.App(Expr.Sym(op), List(a, b)) if op == "+" || op == "∨" =>
        search(a, rules, context, subst, depth + 1, maxDepth, visited)
          .map { case (p, s) =>
            val inlStep = ProofStep.Apply(CatRule("inl", a, goal), s)
            (p :+ inlStep, s)
          }
          .orElse(
            search(b, rules, context, subst, depth + 1, maxDepth, visited).map {
              case (p, s) =>
