...
object HoTTAdvancedTest {
  def main(args: Array[String]): Unit = {
    ...
    val testCases = List(
      ...
      (
        "transport(λz. ×(A(z), B(z)), p, pair(u, v)) = pair(transport(λz. A(z), p, u), transport(λz. B(z), p, v))",
        true
      ),

      // 2. Coproduct type transport
      (
        "transport(λz. +(A(z), B(z)), p, inl(u)) = inl(transport(λz. A(z), p, u))",
        true
      ),

      // 3. Function type transport
      (
        "transport(λz. →(A(z), B(z)), p, f) = λx. transport(λz. B(z), p, f(transport(λz. A(z), inv(p), x)))",
        true
      ),
      ...
      (
        "(P(zero) ∧ P(one) ∧ path(Type, P(zero), P(one))) → ∀i:Interval. P(i)",
        true
      ),

      // 9. Suspension Induction: P(north) and P(south) and ∀a. P(north)=P(south) implies ∀s:Susp. P(s)
      (
        "(P(north) ∧ P(south) ∧ ∀a. path(Type, P(north), P(south))) → ∀s:Susp. P(s)",
        true
      ),
    ...
    val miningGoal =
      TestParser.parse("∀n. ∀m. ∀k. plus(n, plus(m, k)) = plus(m, plus(n, k))")
    prover.prove(miningGoal, maxDepth = 20) match {
      case Right(_) =>
        println("✓ OK: Complex arithmetic solved with dynamic lemmas")
      case Left(trace) =>
    ...
    try {
      ...
      prover.prove(circleGoal, maxDepth = 10) match {
        case Right(_) =>
          println("✓ OK: Circle defined via DSL and induction proved")
        case Left(trace) =>
          println(s"✗ FAIL: Circle induction failed - Reason: ${trace.reason}")
      }
    } catch {
