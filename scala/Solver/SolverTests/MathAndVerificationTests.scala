...
object ArithmeticTests {
  def run(): Unit = {
    ...
    val extendedArithRules = List(
      CatRule("plus_comm", sym("plus")(v("n"), v("m")), sym("plus")(v("m"), v("n")), List(v("n"), v("m")), domain = "arithmetic"),
      CatRule("plus_assoc", sym("plus")(sym("plus")(v("n"), v("m")), v("k")), sym("plus")(v("n"), sym("plus")(v("m"), v("k"))), List(v("n"), v("m"), v("k")), domain = "arithmetic"),
      CatRule("times_0", sym("times")(sym("0"), v("n")), sym("0"), List(v("n")), domain = "arithmetic"),
      CatRule("times_S", sym("times")(sym("S")(v("n")), v("m")), sym("plus")(v("m"), sym("times")(v("n"), v("m"))), List(v("n"), v("m")), domain = "arithmetic")
    )

    val prover = new Prover(ProverConfig(rules = extendedArithRules ++ StandardRules.all, algebras = StandardRules.defaultAlgebras))
    ...
    cases.foreach { case (input, expected) =>
      ...
      prover.prove(goal, maxDepth = 15) match {
        ...
        case Left(_) => if (expected) println("✗ FAIL") else println("✓ OK (Failed as expected)")
      }
    }
  }
}
