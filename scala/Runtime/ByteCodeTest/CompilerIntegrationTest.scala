...
@main def CompilerIntegrationTest(): Unit =
  ...
  def test(name: String, tree: Tree[(String, Vector[String])]): Unit =
    ...
    try {
      ...
      optimizer.verify(rawOps) match
        case Right(_) =>
          println(
            "  ✓ PROOF SUCCEEDED: The code is mathematically guaranteed to be memory-safe."
          )
        case Left(err) =>
  ...
  // --- 5. 命令削減の検証 (1 + 2 * 3) ---
  // AST: Add(Const(1), Mul(Const(2), Const(3)))
  // 期待結果: LoadConst(0, 7) 1命令 + Return(0)
  val test5 = Tree.V(
    ("Add", Vector.empty),
    Vector(
      Tree.V(("Const", Vector("1")), Vector.empty),
      Tree.V(
        ...
        Vector(
          Tree.V(("Const", Vector("2")), Vector.empty),
          Tree.V(("Const", Vector("3")), Vector.empty)
        )
      )
    )
  )
  test("Constant Folding (1 + 2 * 3)", test5)
