  ...
  def execute(code: Array[Op]): Value =
    ...
    if jitCompiled then
       try {
         ...
         case e: Exception => 
           println(s"[SpeculativeExecutor] JIT Execution Error: ${e.getMessage}. Falling back to VM.")
      ...
      try {
        ...
        case e: Exception =>
          println(s"[SpeculativeExecutor] JIT Compilation/Execution Error: ${e.getMessage}. Falling back to VM.")
          vm.runWithProfiling(code)
      }
    else
      vm.runWithProfiling(code)
