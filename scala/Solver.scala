...
enum Prop:
  ...
  override def toString: String = this match
    case Pred(n, args) =>
      if args.isEmpty then n else s"$n(${args.mkString(",")})"
    case Eq(l, r)     => s"$l=$r"
...
object Unifier:
  ...
  def applySubst(proof: Term, s: Subst): Term = proof match
    ...
    case Term.Case(sel, lv, lb, rv, rb) =>
      Term.Case(
        applySubst(sel, s),
        lv,
        applySubst(lb, s),
    ...
    case Term.Unpack(ex, v, p, b) =>
      Term.Unpack(applySubst(ex, s), v, p, applySubst(b, s))
    case Term.Rewrite(eq, t) =>
      Term.Rewrite(applySubst(eq, s), applySubst(t, s))
  ...
  def unifyProp(p1: Prop, p2: Prop, subst: Subst): Option[Subst] =
    (p1, p2) match
      case (Prop.Pred(n1, a1), Prop.Pred(n2, a2))
          if n1 == n2 && a1.length == a2.length =>
        a1.zip(a2).foldLeft(Option(subst)) { case (sOpt, (i1, i2)) =>
          sOpt.flatMap(s => unifyInd(i1, i2, s))
        }
...
object Prover:
  ...
  def prove(target: Prop): Option[Term] =
    metaCounter = 0
    solve(target, Nil, emptySubst, 0).map { case (term, subst) =>
      applySubst(term, subst)
    }
  ...
  private def solve(
    ...
    if depth > 25 then None
      ...
      solveInvertible(currentTarget, ctx, subst, depth)
        .orElse(decomposeContext(ctx, subst, depth).flatMap {
          (newCtx, newSubst, wrapper) =>
            solve(target, newCtx, newSubst, depth).map { (t, s) =>
              (wrapper(t), s)
            }
        })
        .orElse(solveSearch(currentTarget, ctx, subst, depth))

  private def solveInvertible(
      target: Prop,
      ctx: Context,
      subst: Subst,
      depth: Int
    ...
    if target == Prop.True then Some((Term.UnitTerm, subst))
      ...
      target match
        case Prop.Arrow(from, to) =>
          val v = s"p$depth"
          solve(to, (v, from) :: ctx, subst, depth + 1).map { (b, s) =>
            (Term.Abs(v, b), s)
          }
        case Prop.And(l, r) =>
          solve(l, ctx, subst, depth + 1).flatMap { (t1, s1) =>
            solve(r, ctx, s1, depth + 1).map { (t2, s2) =>
              (Term.Pair(t1, t2), s2)
            }
          }
        case Prop.Forall(v, body) =>
          ...
          solve(instantiatedBody, ctx, subst, depth + 1).map { (b, s) =>
            (Term.Abs(xName, b), s)
          }
        case _ => None

  private def solveSearch(
      target: Prop,
      ctx: Context,
      subst: Subst,
      depth: Int
    ...
    val reflRes = target match
      case Prop.Eq(l, r) =>
        unifyInd(l, r, subst).map(s => (Term.Refl(applySubst(l, s)), s))
      case _ => None
      ...
      prop match
        case Prop.Arrow(from, to) =>
          unifyProp(to, target, subst).flatMap { s =>
            solve(from, ctx, s, depth + 1).map { (argP, sFinal) =>
              (Term.App(Term.Var(name), argP), sFinal)
            }
          }
        case _ => None
    }.headOption
    ...
    val forallRes = ctx.view.flatMap { (name, prop) =>
      prop match
        case Prop.Forall(v, body) =>
          ...
          solve(target, (s"inst_$depth", inst) :: ctx, subst, depth + 1).map {
            (t, s) =>
    ...
    val existsRes = target match
      case Prop.Exists(v, body) =>
        val m = freshMeta()
        solve(substIndInProp(body, v, m), ctx, subst, depth + 1).map { (p, s) =>
          (Term.Witness(m, p), s)
        }
      case _ => None
    ...
    val orRes = target match
      case Prop.Or(l, r) =>
        solve(l, ctx, subst, depth + 1)
          .map { (t, s) => (Term.Inl(t), s) }
          .orElse(solve(r, ctx, subst, depth + 1).map { (t, s) =>
            (Term.Inr(t), s)
          })
      case _ => None
    ...
    val rewriteRes = ctx.view.flatMap { (name, prop) =>
      prop match
        case Prop.Eq(l, r) =>
          ...
          if r1 != target then
            solve(r1, ctx, subst, depth + 1).map { (p, s) =>
              (Term.Rewrite(Term.Var(name), p), s)
            }
          else
            val r2 = rewriteProp(target, r, l)
            if r2 != target then
              solve(r2, ctx, subst, depth + 1).map { (p, s) =>
                (Term.Rewrite(Term.Var(name), p), s)
              }
            else None
        case _ => None
  ...
  def decomposeContext(
      ctx: Context,
      subst: Subst,
      depth: Int
  ): Option[(Context, Subst, Term => Term)] =
    ctx.zipWithIndex.collectFirst {
      ...
      case ((name, Prop.Exists(v, body)), i) =>
        ...
        val next = ctx.patch(
          i,
          List((hName, substIndInProp(body, v, Ind.Var(xName)))),
          1
        )
  ...
  def substIndInProp(p: Prop, v: String, t: Ind): Prop = p match
    case Prop.Pred(n, args) =>
      Prop.Pred(n, args.map(a => if a.toString == v then t else a))
    case Prop.Eq(l, r) =>
      Prop.Eq(
        if l.toString == v then t else l,
        if r.toString == v then t else r
      )
    case Prop.Arrow(f, to) =>
      Prop.Arrow(substIndInProp(f, v, t), substIndInProp(to, v, t))
    case Prop.And(l, r) =>
      Prop.And(substIndInProp(l, v, t), substIndInProp(r, v, t))
    case Prop.Or(l, r) =>
      Prop.Or(substIndInProp(l, v, t), substIndInProp(r, v, t))
    case Prop.Forall(v2, b) =>
      if v == v2 then p else Prop.Forall(v2, substIndInProp(b, v, t))
    case Prop.Exists(v2, b) =>
      if v == v2 then p else Prop.Exists(v2, substIndInProp(b, v, t))
    case _ => p

  def replaceVar(t: Term, oldVar: String, newTerm: Term): Term = t match
    case Term.Var(n) if n == oldVar => newTerm
    case Term.Abs(p, b)             =>
      Term.Abs(p, if p == oldVar then b else replaceVar(b, oldVar, newTerm))
    case Term.App(f, a) =>
      Term.App(replaceVar(f, oldVar, newTerm), replaceVar(a, oldVar, newTerm))
    case Term.Pair(a, b) =>
      Term.Pair(replaceVar(a, oldVar, newTerm), replaceVar(b, oldVar, newTerm))
    ...
    case Term.Case(s, lv, lb, rv, rb) =>
      Term.Case(
        replaceVar(s, oldVar, newTerm),
        lv,
        if lv == oldVar then lb else replaceVar(lb, oldVar, newTerm),
        rv,
        if rv == oldVar then rb else replaceVar(rb, oldVar, newTerm)
      )
    case Term.Witness(i, p) => Term.Witness(i, replaceVar(p, oldVar, newTerm))
    case Term.Unpack(ex, v, p, b) =>
      Term.Unpack(
        replaceVar(ex, oldVar, newTerm),
        v,
        p,
        if p == oldVar then b else replaceVar(b, oldVar, newTerm)
      )
    case Term.Rewrite(eq, p) =>
      Term.Rewrite(
        replaceVar(eq, oldVar, newTerm),
        replaceVar(p, oldVar, newTerm)
      )
  ...
  def rewriteProp(p: Prop, from: Ind, to: Ind): Prop = p match
    case Prop.Pred(n, args) =>
      Prop.Pred(n, args.map(a => if a == from then to else a))
    case Prop.Eq(l, r) =>
      Prop.Eq(if l == from then to else l, if r == from then to else r)
    case Prop.Arrow(f, t) =>
      Prop.Arrow(rewriteProp(f, from, to), rewriteProp(t, from, to))
    case Prop.And(l, r) =>
      Prop.And(rewriteProp(l, from, to), rewriteProp(r, from, to))
    case Prop.Or(l, r) =>
      Prop.Or(rewriteProp(l, from, to), rewriteProp(r, from, to))
...
//TODO:PropParserを廃止し、適切なデータ構造に変換して渡すブリッジを構築する。
object PropParser:
  ...
  def parse(input: String): Prop =
    ...
    if s.peek != 0.toChar then
      throw new Exception(s"Unexpected char at ${s.pos}")
    res
  ...
  def parseArrow(s: Scanner): Prop =
    ...
    s.eatWhitespace
    if s.peek == '-' then {
      s.advance; consume(s, '>'); Prop.Arrow(left, parseArrow(s))
    } else left

  def parseOr(s: Scanner): Prop =
    var left = parseAnd(s)
    while { s.eatWhitespace; s.peek == '|' } do {
      s.advance; left = Prop.Or(left, parseAnd(s))
    }
    left
  ...
  def parseAnd(s: Scanner): Prop =
    ...
    while { s.eatWhitespace; s.peek == '&' } do {
      s.advance; left = Prop.And(left, parseQuantifier(s))
    }
    left
  ...
  def parseAtom(s: Scanner): Prop =
    s.eatWhitespace
    if s.peek == '(' then {
      s.advance; val p = parseArrow(s); consume(s, ')'); p
    } else if s.peek == 'T' && !Character.isLetter(peekNext(s)) then {
      s.advance; Prop.True
    } else if s.peek == 'F' && !Character.isLetter(peekNext(s)) then {
      s.advance; Prop.False
    } else
      val name = parseId(s)
      s.eatWhitespace
      if s.peek == '(' then
  ...
  def parseId(s: Scanner): String =
    ...
    while Character.isLetterOrDigit(s.peek) || s.peek == '_' do {
      sb.append(s.peek); s.advance
    }
    if sb.isEmpty then throw new Exception("Expected ID"); sb.toString
  def peekNext(s: Scanner): Char =
    if s.pos + 1 < s.input.length then s.input(s.pos + 1)
    else 0.toChar
  def consume(s: Scanner, c: Char): Unit = {
    s.eatWhitespace
    if s.peek == c then s.advance
    else throw new Exception(s"Expected $c")
  }
/*
