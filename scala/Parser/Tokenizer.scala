...
final class Tokenizer(rules: immutable.Map[String, Regex]) {
  ...
  def toknize(s: String): TokenTree = {
    ...
    val correctedPaths = reversedPaths.map { path =>
      ...
      val tokens =
        if (path.headOption.exists(_._4 == "SOF")) path.tail else path
      var r: UInt = 0
      var c: UInt = 0
  ...
  def toknizeFromSOF(
      s: String
  )(row: Row = 0, col: Col = 0, offset: UInt = 0): Vector[TokenTree] = {
    logger.log(
      s"starting toknizeFromSOF. offset: $offset, s.length: ${s.length}"
    )
      ...
      val rawBranches = (1 to s.length).flatMap { len =>
        if (cache.get((offset + len, true)).exists(_.nonEmpty)) {
          ...
          val new_col: UInt =
            ...
            else col + matchedText.length

          val nextString = s.substring(len)
          val subTrees =
            toknizeFromSOF(nextString)(new_row, new_col, offset + len)
  ...
  def toknizeFromEOF(
    ...
    logger.log(
      s"starting toknizeFromEOF. offset: $offset, s.length: ${s.length}"
    )
