...
object Predicates {
  ...
  def repeat(p: Predicate, n: UInt=0, stopl: Predicate=null): Predicate = new Predicate {
    def check(t: Token): Boolean = {
      if (n!=0){
        boundary {
          var count: UInt= 0
        ...
        throw new IllegalArgumentException("repeat count and stop condition cannot both be 0")
      }
    }
  }
}